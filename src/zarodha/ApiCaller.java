...
public class ApiCaller {
	...
	public void placeOrder(KiteConnect kiteConnect) throws KiteException, IOException {
		 ...
		 * can be regular, bo, co, amo) place order will return order model
		 * which will have only orderId in the order model
		 *
		...
		orderParams.tag = "myTag"; // tag is optional and it cannot be more than
									// 8 characters and only alphanumeric is
									// allowed
	...
	public void placeCoverOrder(KiteConnect kiteConnect) throws KiteException, IOException {
		/**
		 * Cover Order:- following is an example param for the cover order key:
		 * quantity value: 1 key: price value: 0 key: transaction_type value:
		 * BUY key: tradingsymbol value: HINDALCO key: exchange value: NSE key:
		 * validity value: DAY key: trigger_price value: 157 key: order_type
		 * value: MARKET key: variety value: co key: product value: MIS
		 */
		OrderParams orderParams = new OrderParams();
	...
	public void getTriggerRange(KiteConnect kiteConnect) throws KiteException, IOException {
		// You need to send transaction_type, exchange and tradingsymbol to get
		// trigger range.
	...
	public void getOrders(KiteConnect kiteConnect) throws KiteException, IOException {
		...
		for (int i = 0; i < orders.size(); i++) {
			System.out.println(orders.get(i).tradingSymbol + " " + orders.get(i).orderId + " "
					+ orders.get(i).parentOrderId + " " + orders.get(i).orderType + " " + orders.get(i).averagePrice
	 ...
	 * exchange with tradingsymbol or instrument token only. For example
	 * {NSE:NIFTY 50, BSE:SENSEX} or {256265, 265}
	 */
	...
	public void getHistoricalData(KiteConnect kiteConnect) throws KiteException, IOException {
		 ...
		 * interval, continuous (for expired F&O contracts) returns historical
		 * data object which will have list of historical data inside the
		 * object.
		 */
	...
	public void placeMFSIP(KiteConnect kiteConnect) throws KiteException, IOException {
		System.out
				.println("mf place sip: " + kiteConnect.placeMFSIP("INF174K01LS2", "monthly", 1, -1, 5000, 1000).sipId);
	}
	 ...
	 * com.zerodhatech.ticker disconnection
	 */
	public Map<String, LTPQuote> getLtp(KiteConnect kiteConnect, String[] input)
			throws IOException, WebSocketException, KiteException {
		AppConstant constant = AppConstant.getInstance();
		double ltp = 0.00;
		
		String[] input1 = {"NSE:INFY"};
			return kiteConnect.getLTP(input);
		...
		*//** Set error listener to listen to errors. *//*
		...
		tickerProvider.setOnTickerArrivalListener(new OnTicks() {
			...
			public void onTicks(ArrayList<Tick> ticks) {
				...
				System.out.println("ticks size " + ticks.size());
				if (ticks.size() > 0) {
					constant.ltp = ticks.get(0).getLastTradedPrice();
				}
			}
		});
		// Make sure this is called before calling connect.
